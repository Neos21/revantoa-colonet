...
import { contentMaxLength, newAnonymousPostSchema, type NewAnonymousPost } from '../../../../shared/schemas/new-anonymous-post';
...
export default function AnonymousPostForm({ onSubmitted }: Props): ReactNode {
  ...
  const [form, setForm] = useState<NewAnonymousPost>({ content: '' });
  ...
  const onSubmit = async (event: FormEvent): Promise<void> => {
    ...
    const parsed = newAnonymousPostSchema.safeParse(form);
    if(!parsed.success) {
    ...
    try {
      ...
      await ky.post(apiPaths.anonymousPosts.route, { json: requestBody }).json();
